...
Kp = 15.0
Ki = 0.0   # Hint: You will need I-control to offset gravity!
Kd = 5.0
# ==========================================

x_ref = 10.0  # Desired altitude
...
# LQR Dynamics (includes gravity feedforward)
def lqr_system(t, state):
    x, v = state
    u = -K_lqr[0, 0] * (x - x_ref) - K_lqr[0, 1] * v + g
    return [v, u - g]
...
ax.axhline(x_ref, color='r', linestyle=':', label='Setpoint (Altitude)')
...
ax.set_title("Rocket System Response")
ax.set_xlabel("Time (s)")
ax.set_ylabel("Altitude (x)")
ax.legend(loc='lower right')
ax.grid(True)

# Animation
fig_anim, ax_anim = plt.subplots()
ax_anim.set_xlim(-1, 2)
ax_anim.set_ylim(0, 15)
ax_anim.set_title("Rocket Animation")
ax_anim.axhline(x_ref, color='r', linestyle=':', label='Setpoint')
ax_anim.grid(True)

rocket_pid, = ax_anim.plot([], [], '^', markersize=15, color='blue', label='PID')
rocket_lqr, = ax_anim.plot([], [], '^', markersize=15, color='orange', alpha=0.6, label='LQR')
ax_anim.legend()

def init():
    rocket_pid.set_data([], [])
    rocket_lqr.set_data([], [])
    return rocket_pid, rocket_lqr

def animate(i):
    rocket_pid.set_data([0], [sol_pid.y[0, i]])
    rocket_lqr.set_data([1], [sol_lqr.y[0, i]])
    return rocket_pid, rocket_lqr
